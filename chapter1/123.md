...
3. **Lock**  
   ...
   **ReentrantLock 可重入锁：**
   ...
   1. 可重入锁就是一个线程在获取了锁之后，可再次去获取同一个锁。可重入锁也叫做递归锁，同一线程 外层函数获得锁之后 ，内层递归函数仍然有获取该锁的代码，则不需要重新申请锁，可以直接执行。在JAVA环境下 ReentrantLock 和synchronized 都是 可重入锁。

   2. 线程使用ReentrantLock获取锁分为两个阶段

      1. **第一个阶段是初次竞争：**在初次竞争的时候是否考虑队列节点直接区分出了公平锁和非公平锁。

      2. **第二个阶段是基于CHL队列的竞争。**在基于CHL队列的锁竞争中，依靠CAS操作保证原子操作，依靠LockSupport来做线程的挂起和唤醒，使用队列来保证并发执行变成了串行执行，从而消除了并发所带来的问题。

   ReentrantLock支持两种获取锁的方式：**默认使用非公平**

   1. **公平模型**：线程A释放锁后，state恢复为0，然后通知队列唤醒B线程。对于公平锁，如果CLH队列有节点，则新进入竞争的线程一定要在CLH上排队。

   2. **非公平模型**：非公平锁则是无视CLH队列中的节点，直接进行竞争抢占，这就有可能导致CLH队列上的节点永远获取不到锁
